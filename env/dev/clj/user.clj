(ns user
  ...
  (:require
    ...
    [kit.api :as kit]
    [org.ninerian.graal-test.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (org.ninerian.graal-test.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (org.ninerian.graal-test.config/system-config {:profile :test})
                                  (ig/prep)))))
